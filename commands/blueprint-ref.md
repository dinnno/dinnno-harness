---
description: docs/references/_INDEX.md의 등록된 entry를 codex:rescue로 구현 가능 수준 청사진 분석 → blueprints/{name}.md 저장, 메인엔 경로+1줄만 적재. 미등록 URL·단순 요약에는 쓰지 않는다(등록은 /add-ref, 요약은 /harness).
---
...
- 같은 name이 여러 섹션에 있으면 어느 섹션인지 사용자에게 물어 확정.
...
- `_INDEX.md`에 없는 URL 즉석 분석 ❌ (먼저 `/add-ref` 등록).
- 메인 세션이 직접 URL fetch/PDF 읽기 ❌ (codex:rescue 전용 — 토큰·저작권 회피).
- 자동 재시도 ❌ (실패 시 사용자 보고).
- summary 생성(=`/harness` 흐름)·`_GUIDE.md` 수정 ❌.
