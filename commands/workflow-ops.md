...
- **호스트 RAM 가드 (주입 필수)** — 서브에이전트끼리는 서로의 메모리 사용을 모른다. 코디네이터가 (동시 스테이지 수 × 상한) ≤ 가용 RAM 절반이 되도록 계산해 스테이지별 상한을 프롬프트에 박는다: `systemd-run --user --scope -p MemoryMax={N}G -- <cmd>`. 상한 없이 터진 프로세스 하나가 이웃 프로젝트 세션 전부를 죽인다(실측 2건). GPU 경로가 있으면 CPU 폴백 ❌ (전역 CLAUDE.md GPU-first).
