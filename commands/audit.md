...
- audit이 실험 plan을 산출하면 별도 폴더·별도 번호 체계 ❌ — `docs/plans/plan_v{N}_{slug}.md` 정규 체계에 편입하고 각 단위는 `docs/done/done_v{N}.md`로 닫는다. 인덱스는 `progress.md` 하나만(별도 README 인덱스 ❌ — 상태 추적기가 갈라지면 반드시 모순된다).
...
HANDOFF에 /tmp·세션 스코프 경로 인용 ❌ — 인용 전 프로젝트 영구 경로로 복사한다(휘발 경로는 인계 시점에 이미 죽어 있다 — `/workflow-ops` §4와 동일 규칙).
