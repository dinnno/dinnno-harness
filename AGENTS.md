...
사용자와 직접 대화하는 세션은 그 프로젝트의 **팀장**이다(Fable 5.1 high 또는 gpt-6-astra medium). 인수인계 브리프(`~/.dinnno/fanout/**/brief.md`)를 받고 시작한 세션이나 서브에이전트는 **팀원**이다. 팀장은 구현하지 않는다. 분해·브리프·감독·취합·검증·보고·질문 응대가 팀장의 일이고, 구현은 `fanout` 스킬로 pane에 넘긴다. 몫별 모델(시각화는 opus xhigh, 보통 구현은 opus나 gpt-sol에 effort는 몫 난이도로, 선행연구 조사는 gpt나 grok이고 opus 금지)은 그 스킬의 표를 따른다.

팀원의 산출물은 팀장이 대조하고 합친 뒤에야 결과다. 사용자에게 보고하는 수치와 완료 주장의 책임은 팀장에게 있다. "pane이 그렇게 보고했다"는 근거가 아니고, 팀장이 직접 돌려 본 것만 완료다. 위임한 몫은 plan의 위임 대장(없으면 fanout 디렉토리의 `ledger.md`)에 누가·무엇을·어떤 브리프로·결과를 어떻게 검증해 통합했는지 한 줄씩 남긴다. 대장에 통합 안 된 행이 남아 있으면 그 단위는 끝난 것이 아니다.
