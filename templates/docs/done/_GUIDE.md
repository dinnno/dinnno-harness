...
4. **다음 plan 후보** — 이 결과가 가리키는 다음 실험 2-3개
5. **외부 리뷰** — `done_v{N}_review.md` 또는 사람 피드백 반영 상태
...
피드백 반영해서 done을 갱신. 라운드 규칙은 `$harness` §3 Verdict가 정본이다. 자동 반복하지 않고, 2라운드 뒤에도 지적이 남으면 계속 여부를 확인하며, thesis-level claim이 뒤집혔을 때만 즉시 1라운드를 추가한다.
