---
name: improve
description: Isolated, read-only engineering advisor for broad codebase friction that is materially slowing research. Use for a focused engineering audit, not a dinnno scientific/project audit, ordinary code review, or normal small task.
license: MIT
metadata:
  author: shadcn
  version: "1.0.0"
...
## Dinnno isolation adapter

In a dinnno research project, use only Recon, Audit, and Vet to produce evidence-backed advisory findings. Stop after Phase 3: do not run Phase 4 or the `execute`, `reconcile`, or `--issues` variants. The primary agent must reopen and verify cited source; a selected finding becomes a separate bounded `/harness` unit. This skill has no authority over thesis, hypotheses, comparison axes, evaluation protocol, Loop 1/2 transitions, experiment budgets, real-robot actuation, destructive operations, or kill/NO-GO decisions.
