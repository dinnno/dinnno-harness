...
런처가 vault root(`SECOND_BRAIN_ROOT` → CWD 상위 탐색 → 알려진 후보)와 pyyaml 가능한 인터프리터를 알아서 잡고, 마지막 fetch가 1시간을 넘었으면 원격을 fast-forward로 받아온다(로컬 편집은 덮지 않는다). pull이 실패하면 몇 커밋 뒤처졌는지 stderr로 알린다. 경로를 직접 쓰지 말 것.
