...
description: >
  ...
  말하지 않았더라도 이 방식이 나으면 제안한다. 목적은 큰 구현 토큰을 pane이 쓰게 하고 팀장은
  분해·감독·취합·검토에 집중하는 것이다(30분 안의 작은 몫은 팀장이 직접). herdr 안(HERDR_ENV=1)에서만
...
4. **위임 대장에 적는다.** 항상 `$FO/ledger.md`에(진행 중 plan이 있으면 §7 위임 대장에도) pane마다 한 줄: 이름 · 모델 · 브리프 경로 · report 경로 · 팀장이 돌린
   검증 · 통합. 통합 열 값은 `반영`, `반려: 사유`, `대기` 중 하나다. 통합 열이 빈 행이 남아 있으면 이 fanout은 끝난 것이
