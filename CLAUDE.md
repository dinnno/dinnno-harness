...
**진입점 통일:** dinnno-harness 깐 프로젝트에서는 모든 작업이 `/harness`로 진입. `/harness` 없이 시작된 요청도 본문 전체(§경계선 포함 §1–§5)를 묵시 적용한다(현황 적재·단위 confirm·세션 종료 계약 포함). (예외: `/audit`과 그 `HANDOFF_TO_OPUS.md` 인계 세션, `/issue` 파일만 읽고 재시도하는 세션, `/deep-clean` 대청소 세션은 해당 문서의 워크플로를 따른다.) Grok 세션은 native `/harness` skill의 adapter 규칙을 따르고 `/opus-guide`를 로드하지 않는다. 그 밖의 현재 세션 모델이 Fable/Mythos 계열이 아니면(Opus·Sonnet 등) 다른 작업을 시작하기 전에 `/opus-guide`를 먼저 로드한다.
