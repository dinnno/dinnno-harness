...
- 2026-08-08 — 아이디어 채널 대칭화: `/harness` §2 💡 인박스에 Claude 제안 착지 허용(`[claude]` 태그+근거 1줄, 세션당 1~2개, 착지≠채택), §3 Setup confirm에 최강 대안 1개 동봉, progress.md 템플릿 착지 규칙 ⑤ 추가 — 조치: 없음(기존 progress.md 주석은 선택 반영)
- 2026-08-08 — docs 위생 2종: `/harness` §2에 교훈 즉시 착지(사용자 지적 → 그 턴에 LEARNINGS 1줄 append + 쓴 줄 표시, LEARNINGS 템플릿 헤더에도 명시), `/tidy` §5 stale 검사 신설(현행 문서 안 낡은 줄 — 문서 간 대조 판정, confirm 후 반영) — 조치: 없음(커맨드 즉시, 템플릿 헤더는 선택 반영)
- 2026-08-08 — fresh 리뷰 일괄 수리 15건: 리뷰 라운드 정본 통일(done/_GUIDE·템플릿 → `/harness` §4), 전역 우선순위에 커맨드 계층 삽입, 묵시 적용 §1–§5 통일(+`/issue` 재시도 세션 예외), vault 경로 fallback, `/harness` §1에 references pending 스캔·진행 중 plan 적재 복원, 당일 싱크 재발동 종결 조건, 경계선⑦ 실패 정의 내장, §3 autoloop push 예외(LOOP 우선), `/tidy` 정규 집합에 LOOP.md, stale 정리 confirm 명시, `/issue` 트리거 수락 명시, CHANGELOG 날짜순 복구 — 잔여 소형 7건은 `BACKLOG.md` — 조치: 없음
...
- 2026-08-08 — 2차 최종 검증 수리 7건: `last-sync`를 날짜 → 마지막 반영 항목 마커로(당일 다중 항목 영구 누락 홀 봉합 — templates/CLAUDE.md 형식 동반 변경), 묵시 적용 범위에 §경계선 명시(전역·템플릿), opus-guide 우선순위에 커맨드 계층 전파, `/close` 제안·수락 동형화 + 검토 입력을 plan 전체로 정정, docs/_GUIDE 구명칭 "(b)" 정정, 💡 평가 금지에 전역 push-back 예외 명시 — 잔여 소형 7건 `BACKLOG.md` 추가 — 조치: 기존 프로젝트는 다음 싱크 때 last-sync 형식만 갱신
